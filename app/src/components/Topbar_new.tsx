  ...
  return (
    <div className="border-b bg-neutral-50 dark:bg-neutral-900 px-4 py-2 flex items-center gap-3">
      ...
      <div className="flex items-center gap-2 flex-1 min-w-0">
        ...
        <select className="px-2 py-1 rounded border bg-white/70 dark:bg-neutral-800 text-sm cursor-pointer" value={profile ?? ''} onChange={e => { const next = e.target.value || undefined; setIsSwitchingProfile(true); setConnected(false); setProfile(next); selectBucket(undefined); queryClient.clear(); setError(''); setConnectionError(undefined); if (next) { void connect(next) } else { setStatus('Not connected'); setIsSwitchingProfile(false) } }}>
          <option value="">(none)</option>
          {profiles.map(p => (
            <option key={p.name} value={p.name}>{p.name}{p.isSso ? ' (SSO)' : ''}</option>
          ))}
        </select>
        <button aria-label="Refresh profiles" title="Refresh profiles" className="p-1 rounded hover:bg-neutral-200 dark:hover:bg-neutral-800 disabled:opacity-50 cursor-pointer" disabled={refreshing} onClick={() => { trace('ui', 'refresh profiles'); void refreshProfiles() }}>
          {refreshing ? (
      ...
      <div className="pl-2">
        <button className="px-2 py-1 text-sm rounded bg-neutral-200 dark:bg-neutral-800 cursor-pointer" onClick={openSettings}>
          Settings
        </button>
      </div>
    </div>
